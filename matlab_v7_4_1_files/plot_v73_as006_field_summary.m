function h = plot_v73_as006_field_summary(spec, field, expField, score)
%PLOT_V73_AS006_FIELD_SUMMARY Thesis-style AS006 field-map comparison.

if nargin < 4
    score = struct('available', false, 'combined', NaN);
end

h = figure('Name', 'v7.3 AS006 magnetic-field dVdI maps', ...
...
title('field suppression / switching');
...
sgtitle(sprintf('v7.3 %s out-of-plane field scaffold: %s, %s', ...
...
ylabel('max switched-link fraction');
ylim([0 1]);
