%RUN_V744_AS006_PHYSICAL_BOTTLENECK_SWEEP Controlled physical W_ij ablation.
%
% v7.4.4 keeps geometry, PDE/Raman proxy, Tc field, disorder realization,
...
projectDir = add_v744_paths();
outDir = fullfile(projectDir, 'outputs', 'v7_4_4_as006_physical_bottleneck');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
screenFieldOpts = make_screening_field_options_v744(fullFieldOpts, expField, opts);
cases = make_v744_bottleneck_cases(opts);
scoreCsvPath = fullfile(outDir, 'AS006_v7_4_4_physical_bottleneck_scores.csv');

fprintf('\nRunning v7.4.4 physical bottleneck W_ij ablation sweep for %s\n', device);
...
baseParamsUncal = assign_link_parameters(netPDE, spec, modelParams, seed, ...
    'v7_4_4_physical_bottleneck_base');

[fullScaleTable, fullReference, paramsFullRef, weakFullRef] = ...
    build_full_scale_table_v744(cases, netPDE, spec, baseParamsUncal, opts);
...
if isempty(sweepTable)
    sweepRows = repmat(empty_row_v744(), expectedRows, 1);
    rowIdx = 0;
    ...
    for k = 1:numel(cases)
        ...
        for mode = ["shape","conductance"]
            ...
            if mode == "conductance"
                normalScaleForMode = normal_scale_for_case_v744(caseInfo, ...
                    fullScaleTable, referenceFullNormalScale);
            end
            [netCase, paramsCase, weak] = build_v744_bottleneck_case(netPDE, spec, ...
            ...
            fieldScreen.version = sprintf('v7.4.4 %s screening', mode);

            shapeScore = compute_v742_weaklink_feature_score(fieldScreen, ...
                expField, opts.shapeScore);
            conductanceScore = compute_v744_conductance_score(fieldScreen, ...
                expField, opts.conductanceScore);

            sweepRows(rowIdx) = make_sweep_row_v744(spec, caseInfo, weak, ...
...
hSweep = plot_v744_bottleneck_summary(sweepTable);
export_chapter_figure(hSweep, outDir, 'AS006_v7_4_4_physical_bottleneck_summary');
...
if opts.runFullFieldMaps
    ...
    export_chapter_figure(hShape, outDir, ...
        sprintf('AS006_v7_4_4_best_shape_%s_field_maps', char(bestShape.caseName)));
    ...
    export_chapter_figure(hCond, outDir, ...
        sprintf('AS006_v7_4_4_best_conductance_%s_field_maps', char(bestConductance.caseName)));
else
    fprintf('\nSkipping full 121x121 field maps because opts.runFullFieldMaps = false.\n');
    fprintf('Set opts.runFullFieldMaps = true in make_v744_bottleneck_options.m to export full maps for the best cases.\n');
end

save(fullfile(outDir, 'AS006_v7_4_4_physical_bottleneck.mat'), ...
...
fprintf('\nFinished v7.4.4 physical bottleneck W_ij ablation.\n');
...
function opts = make_screening_field_options_v744(fullOpts, expField, sweepOpts)

opts = fullOpts;
opts.version = 'v7.4.4 screening';
...
if isempty(fullIdx)
    ...
    [~, paramsFullRef, weakFullRef] = build_v744_bottleneck_case(netPDE, spec, ...
...
for r = 1:numel(fullIdx)
    ...
    [~, paramsFull, weakFull] = build_v744_bottleneck_case(netPDE, spec, ...
...
function normalScale = normal_scale_for_case_v744(caseInfo, fullScaleTable, referenceScale)
...
function row = make_sweep_row_v744(spec, caseInfo, weak, mode, shapeScore, conductanceScore)
...
row.shapeFullMap = mean_pair_component_v744(shapeScore.fullMapRms);
row.shapeLowBias = mean_pair_component_v744(shapeScore.lowBiasRms);
row.shapeZeroBias = mean_pair_component_v744(shapeScore.zeroBiasRms);
row.shapeAsymmetry = shapeScore.asymmetryRms;

row.conductanceScore = conductanceScore.combined;
row.conductanceFullMap = mean_pair_component_v744(conductanceScore.fullMapRms);
row.conductanceLowBias = mean_pair_component_v744(conductanceScore.lowBiasRms);
row.conductanceZeroBias = mean_pair_component_v744(conductanceScore.zeroBiasRms);
...
function val = mean_pair_component_v744(s)
...
if isempty(idx)
    error('Could not find v7.4.4 case "%s".', caseName);
end
caseInfo = cases(idx);
...
field.version = sprintf('v7.4.4 %s full', mode);
shapeScore = compute_v742_weaklink_feature_score(field, expField, opts.shapeScore);
conductanceScore = compute_v744_conductance_score(field, expField, opts.conductanceScore);
...
s.message = sprintf('v7.4.4 %s score', label);
