function score = compute_v744_conductance_score(field, expField, opts)
%COMPUTE_V744_CONDUCTANCE_SCORE Absolute-scale dV/dI score.
%
...
score.definition = ['v7.4.4 conductance-preserving feature score. ', ...
...
for kp = 1:numel(pairs)
    ...
    expRN = percentile_local_v744(expRaw(isfinite(expRaw)), 95);
    modelRN = percentile_local_v744(field.dVdI.(pairName)(isfinite(field.dVdI.(pairName))), 95);
    score.expRN.(pairName) = expRN;
    score.modelRN.(pairName) = modelRN;

    modelScaled = field.dVdI.(pairName) ./ max(expRN, eps);
    expScaled = interp_exp_to_model_v744(expField, expRaw ./ max(expRN, eps), field);
    valid = isfinite(modelScaled) & isfinite(expScaled);
    fullRms = rms_valid_v744(modelScaled(valid) - expScaled(valid));

    lowMask = abs(field.I_A(:)) <= opts.lowBiasWindow_A;
    lowMask2 = repmat(lowMask, 1, numel(field.B_T));
    lowValid = valid & lowMask2;
    lowBiasRms = rms_valid_v744(modelScaled(lowValid) - expScaled(lowValid));

    zbModel = zero_bias_trace_v744(field.I_A, modelScaled);
    zbExp = zero_bias_trace_v744(field.I_A, expScaled);
    zeroBiasRms = rms_valid_v744(zbModel - zbExp);
...
if isfield(field.dVdI, 'top_4_10') && isfield(field.dVdI, 'bottom_3_9') && ...
    ...
    expTop = get_exp_raw_v744(expField, 'top_4_10');
    expBot = get_exp_raw_v744(expField, 'bottom_3_9');
    rnTop = percentile_local_v744(expTop(isfinite(expTop)), 95);
    rnBot = percentile_local_v744(expBot(isfinite(expBot)), 95);
    mt = field.dVdI.top_4_10 ./ max(rnTop, eps);
    mb = field.dVdI.bottom_3_9 ./ max(rnBot, eps);
    et = interp_exp_to_model_v744(expField, expTop ./ max(rnTop, eps), field);
    eb = interp_exp_to_model_v744(expField, expBot ./ max(rnBot, eps), field);
    modelDelta = mb - mt;
    expDelta = eb - et;
    lowMask = abs(field.I_A(:)) <= opts.lowBiasWindow_A;
    lowMask2 = repmat(lowMask, 1, numel(field.B_T));
    valid = isfinite(modelDelta) & isfinite(expDelta) & lowMask2;
    score.asymmetryRms = rms_valid_v744(modelDelta(valid) - expDelta(valid));
end
...
if ~isempty(pairScores)
    ...
    score.message = sprintf('v7.4.4 conductance score from %d pair(s).', numel(pairScores));
else
...
function expRaw = get_exp_raw_v744(expField, pairName)
if isfield(expField, 'R') && isfield(expField.R, pairName)
    expRaw = expField.R.(pairName);
else
    expRaw = expField.Rnorm.(pairName);
...
function p = percentile_local_v744(vals, pct)
