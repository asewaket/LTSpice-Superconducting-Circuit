...
For the v7.3 AS006 out-of-plane magnetic-field `dV/dI(I,B)` scaffold:
...
For the v7.4.2 focused AS006 weak-link ablation sweep:
...
For the v7.4.4 physical-bottleneck `W_ij` sweep:
...
For the v7.7 multi-observable ranking of real v7.4.x candidate outputs:

```matlab
cd matlab_v7_7_files
run_v77_multiobservable_scoring
```

For the v7.7.1 mechanism-evidence report:
