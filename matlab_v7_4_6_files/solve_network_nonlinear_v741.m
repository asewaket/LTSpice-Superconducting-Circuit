function [v, gx, gy] = solve_network_nonlinear_v741(net, spec, params, T, Iapp, gxInit, gyInit)
%SOLVE_NETWORK_NONLINEAR_V741 Fixed-point solve with weak-link switching.
...
for iter = 1:spec.solver.maxIter
    ...
    [RxNew, RyNew] = link_resistance_TI_v741(params, spec, T, abs(Ix), abs(Iy));
...
warning('v7.4.1 nonlinear solve reached maxIter for %s at T=%.3g K, I=%.3g A.', ...
    spec.name, T, Iapp);
[v, ~, ~] = solve_network_linear(net, gx, gy, Iapp);
