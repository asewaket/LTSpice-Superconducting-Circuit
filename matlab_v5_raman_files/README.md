# MoTe2 superconducting network MATLAB scaffold, v5

This folder is the fifth iteration of the MATLAB project built from
`Res_Array_1.m`.
...
v5 keeps the v4 network/proxy/ensemble/figure/metric structure and adds a first
Raman-data layer:
...
masks. v5 adds Raman line-scan constraints, but these are not yet registered
onto the Hall-bar network grid. The next step is to define scan start/end
positions, scan direction, and how each scan should be interpolated onto the
2D network.
