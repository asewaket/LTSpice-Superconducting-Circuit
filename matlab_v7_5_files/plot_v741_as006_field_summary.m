...
versionLabel = 'v7.4.1';
if isfield(field, 'version') && strlength(string(field.version)) > 0
    versionLabel = char(string(field.version));
end

h = figure('Name', sprintf('%s AS006 weak-link magnetic-field dVdI maps', versionLabel), ...
...
sgtitle(sprintf('%s %s weak-link out-of-plane field scaffold: %s, %s', ...
    versionLabel, spec.name, spec.filmForceLabel, scoreText), ...
    'FontWeight', 'bold');
