...
if ~isfield(opts, 'minCoverageNorm')
...
minDistance_um = inf(size(net.active));
...
coverage(~net.active) = 0;
if max(coverage(:)) > 0
    coverageNorm = coverage ./ max(coverage(:));
else
    coverageNorm = coverage;
end
coverageNorm(~net.active) = NaN;

supportedMask = net.active & isfinite(nodeMapNorm) & ...
    coverageNorm >= opts.minCoverageNorm;
nodeMapDisplay = nodeMapNorm;
nodeMapDisplay(~supportedMask) = NaN;
...
ramanMap.nodeMapDisplay = nodeMapDisplay;
ramanMap.coverage = coverage;
ramanMap.coverageNorm = coverageNorm;
ramanMap.supportedMask = supportedMask;
ramanMap.minDistance_um = minDistance_um;
