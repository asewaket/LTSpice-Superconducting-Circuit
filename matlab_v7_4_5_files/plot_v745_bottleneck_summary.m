function h = plot_v745_bottleneck_summary(sweepTable)
%PLOT_V745_BOTTLENECK_SUMMARY Summary of physical bottleneck W_ij ablations.

if isempty(sweepTable)
    error('plot_v745_bottleneck_summary requires a nonempty table.');
end

h = figure('Name', 'v7.4.5 physical bottleneck Wij summary', ...
...
sgtitle('v7.4.5 AS006 physical bottleneck W_{ij} sweep');
apply_v745_summary_light_style(h);
...
ylabel(ylab);
grid on;
if strcmp(scoreName, 'conductanceScore')
    finiteVals = T.(scoreName);
    finiteVals = finiteVals(isfinite(finiteVals) & finiteVals > 0);
    if ~isempty(finiteVals) && max(finiteVals) ./ max(min(finiteVals), eps) > 20
        set(gca, 'YScale', 'log');
...
grid on;
if strcmp(scoreName, 'conductanceScore')
    finiteVals = vals(isfinite(vals) & vals > 0);
    if ~isempty(finiteVals) && max(finiteVals) ./ max(min(finiteVals), eps) > 10
        set(gca, 'YScale', 'log');
        ylabel('best score, lower is better (log scale)');
    end
end

end

function apply_v745_summary_light_style(h)
%APPLY_V745_SUMMARY_LIGHT_STYLE Local style guard for thesis-ready output.
%
