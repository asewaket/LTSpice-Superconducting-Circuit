...
%PLOT_PHYSICS_CASE_RT_PERCOLATION Best v6.7 R(T) plus connectivity.

valid = arrayfun(@(s) isfield(s, 'run') && ~isempty(s.run), bestRuns);
bestRuns = bestRuns(valid);
nDev = numel(bestRuns);

h = figure('Name', 'v6.7 R(T) and percolation comparison', 'Color', 'w', ...
...
for d = 1:nDev
    ...
    plot(Tmod, RmodNorm, '-', 'LineWidth', 2.2, ...
        'Color', [0.0 0.45 0.74], 'DisplayName', 'best v6.7 case');
