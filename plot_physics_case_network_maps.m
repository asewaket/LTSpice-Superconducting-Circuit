...
%PLOT_PHYSICS_CASE_NETWORK_MAPS Spatial maps for best v6.5 cases.

valid = arrayfun(@(s) isfield(s, 'run') && ~isempty(s.run), bestRuns);
bestRuns = bestRuns(valid);
nDev = numel(bestRuns);

h = figure('Name', 'v6.5 physics case network maps', 'Color', 'w', ...
...
for d = 1:nDev
    ...
    nexttile;
    caseNode = run.caseInfo.domainNode | run.caseInfo.contactNode;
    imagesc(net.x_um, net.y_um, double(caseNode));
    set(gca, 'YDir', 'normal');
    axis image tight; caxis([0 1]); colorbar;
    title(sprintf('%s active case mask', run.physicsCase.caseID), 'Interpreter', 'none');
    xlabel('x [\mum]'); ylabel('y [\mum]');
    hold on; draw_geometry_overlays(net, run.spec);
    apply_light_figure_style(gca);
