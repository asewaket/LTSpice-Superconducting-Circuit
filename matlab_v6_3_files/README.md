# MoTe2 superconducting network MATLAB scaffold, v6.3

This folder is the v6.3 iteration of the MATLAB project built from
`Res_Array_1.m`.
...
v6.3 adds explicit A4g/out-of-plane-sensitive Raman mode-ablation tests now
that A4g-labeled datasets are available for AS002, AS005, and AS006.

New v6.3 additions:

- AS002 and AS006 A4g/out-of-plane-sensitive Raman input files;
- explicit `A4g_only` testing for all three devices;
- `in_plane_only = A5g + B2g`;
- `all_with_A4g = A4g + A5g + B2g`;
- leave-one-out mode tests: `without_A4g`, `without_A5g`, `without_B2g`;
- an out-of-plane contribution index comparing all-modes, A4g-only,
  in-plane-only, and without-A4g scores;
- thesis-facing mode-ablation and best-R(T) comparison figures.

Important caveat: the AS002 and AS006 A4g input files note that the
high-frequency Ag branch may be labeled differently under alternate
few-layer mode-numbering conventions. v6.3 therefore treats A4g as an
out-of-plane-sensitive labeled Raman channel for model comparison, not as a
calibrated independent strain-tensor component.

The central v6.3 question is:

```text
Does the A4g/out-of-plane-sensitive Raman channel carry transport-relevant
information beyond the in-plane Raman channels?
```
...
## v6.3 out-of-plane mode-ablation entry point
...
The script uses the v6.3 Raman data folder, which now includes:

```text
data/raman_digitized/AS002_A4g_out_of_plane_raman_digitized.txt
data/raman_digitized/AS006_A4g_out_of_plane_raman_digitized.txt
```

alongside the earlier AS002, AS005, and AS006 Raman datasets.

Outputs are written to:

```text
outputs/out_of_plane_mode_ablation/
```

Important outputs:

```text
out_of_plane_mode_ablation_scores.csv
out_of_plane_mode_summary.csv
out_of_plane_contribution_index.csv
out_of_plane_mode_ablation_summary.png/.pdf
out_of_plane_best_rt_comparison.png/.pdf
out_of_plane_mode_ablation.mat
```

Useful interpretation columns:

```text
delta_without_A4g_minus_all
delta_inplane_minus_all
delta_A4g_minus_inplane
```

Positive `delta_without_A4g_minus_all` means removing A4g worsens the score
relative to all modes, suggesting that the A4g channel contains useful
transport-relevant spatial information. If `A4g_only` beats `in_plane_only`,
that is a stronger indication that the out-of-plane-sensitive channel is
important by itself.
