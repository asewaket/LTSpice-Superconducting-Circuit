function h = plot_v7_as006_pde_summary(spec, netPDE, pde, pdeMap, result, expData, scoreInfo)
%PLOT_V7_AS006_PDE_SUMMARY Thesis-facing diagnostic for first v7 scaffold.

h = figure('Name', 'v7.1 AS006 PDE-informed scaffold', 'Color', 'w', ...
    'Position', [60 60 1320 850]);
...
nexttile;
plot_rt_comparison(result, expData, scoreInfo);

sgtitle(sprintf('v7.1 AS006 PDE-informed scaffold: %s', spec.filmForceLabel), ...
    'FontWeight', 'bold');
...
function plot_rt_comparison(result, expData, scoreInfo)
...
if expData.available && isfield(expData.R, 'main_4p')
    ...
    plot(Texp, RexpNorm, 'ko', 'MarkerSize', 4, ...
        'DisplayName', sprintf('experiment main (%s)', expData.modelPair));
end
...
for k = 1:numel(pairs)
    ...
    if isfield(result.R4p, pairName)
        ...
        plot(Tmod, RmodNorm, '-', 'LineWidth', 2.4, ...
            'Color', pairColors(k,:), ...
            'DisplayName', sprintf('model %s', pairName));
    end
    ...
    if ~isempty(Tp) && ~isempty(Rp)
        ...
        plot(Tp, RpNorm, '.', 'MarkerSize', 9, ...
            'Color', pairColors(k,:), ...
            'DisplayName', sprintf('raw exp %s', pairName));
    end
end
...
ylabel('R/R_N');
ylim([0 1.15]);
title(sprintf('AS006 normalized R(T), selected score %.3g', scoreInfo.combinedScore));
grid on; box on;
apply_light_figure_style(gca);
