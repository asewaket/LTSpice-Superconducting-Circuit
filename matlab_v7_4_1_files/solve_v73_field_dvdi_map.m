function field = solve_v73_field_dvdi_map(net, spec, params0, opts)
%SOLVE_V73_FIELD_DVDI_MAP Simulate dV/dI(I,B) for fixed temperature.
...
for kB = 1:nB
    ...
    if kB == 1 || mod(kB, 10) == 0 || kB == nB
        fprintf('  v7.3 field dV/dI map: B step %d/%d, B = %.4g T\n', ...
            kB, nB, B);
    end
    ...
    for kI = 1:nI
        Iapp = I_vec(kI);
        [v, gxPrev, gyPrev] = solve_network_nonlinear(net, spec, paramsB, ...
...
field.definition = ['Fixed-temperature out-of-plane field sweep. Field ', ...
    ...
    'contribution before the nonlinear current solve.'];
