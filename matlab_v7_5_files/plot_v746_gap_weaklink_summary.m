function h = plot_v746_gap_weaklink_summary(sweepTable)
%PLOT_V746_BOTTLENECK_SUMMARY Summary of gap weak-link gap-tied W_ij ablations.

if isempty(sweepTable)
    error('plot_v746_gap_weaklink_summary requires a nonempty table.');
end

h = figure('Name', 'v7.4.6 gap weak-link Wij summary', ...
...
sgtitle('v7.4.6 AS006 gap weak-link W_{ij} sweep');
apply_v746_summary_light_style(h);
...
function apply_v746_summary_light_style(h)
%APPLY_V746_SUMMARY_LIGHT_STYLE Local style guard for thesis-ready output.
%
